...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	...
	if [[ $(ip -4 addr | grep inet | grep -vEc '127(\.[0-9]{1,3}){3}') -eq 1 ]]; then
		...
		# Automatically set primary IPv4 if more than one is found
	...
	if [[ $(ip -6 addr | grep -c 'inet6 [23]') -gt 1 ]]; then
		# Automatically set primary IPv6 if more than one is found
		ip6=$(ip -6 addr | grep 'inet6 [23]' | cut -d '/' -f 1 | grep -oE '([0-9a-fA-F]{0,4}:){1,7}[0-9a-fA-F]{0,4}' | head -1)
	fi
	# Disable prompts and force these values
	protocol="tcp"
	port="1194"
	dns="3"          # Cloudflare DNS
	client="ahmedVPN"
	# Comment out any lines prompting the user
	# echo
